...
List Gibbs_Posterior(arma::mat Group, arma::mat M, arma::mat T, 
  ...
  for (int l = 0; l < chain; ++l) {
    ...
    for (int i = 0; i < iter; ++i) {
      ...
      for (int m = 0; m < p; ++m) {
        double a = R::dnorm(alphaT(m), 0, std::sqrt(SalphaT1),false) * PalphaT;
        double b = R::dnorm(alphaT(m), 0, std::sqrt(SalphaT0),false) * (1 - PalphaT);
        PIPalphaT(m) =  a / (a+b);
      ...
      //sampling PalphaT
      PalphaT = R::rbeta(1 + arma::accu(IalphaT), p - arma::accu(IalphaT));
      
      //sampling SalphaT1 and SalphaT0
      SalphaT1 = 1/(R::rgamma(arma::accu(IalphaT) / 2 + 1, 1/(arma::accu(arma::pow(IalphaT % alphaT,  2)) / 2 + 1)));
      SalphaT0 = 1/(R::rgamma((p - arma::accu(IalphaT)) / 2 + 1, 1/(arma::accu(arma::pow((1-IalphaT) % alphaT,  2)) / 2 + 10E-4)));
    ...
    //Assign CpG names as rownames
    DataFrame df_PIPa = DataFrame::create(Named("PIPalphaT") = PIP_a);
    CharacterVector CpG(p);
    for(int i = 0; i < p; ++i){
      CpG[i] = "cg" + std::to_string(i + 1);
    }
    df_PIPa.attr("row.names") = CpG;
      
    //select PIP of alphaT>0.5
    arma::uvec select1 = arma::find(PIP_a > 0.5);
    
    //subset posterior distribution selected M
    pos_alphaT = pos_alphaT.cols(select1);
    
    //subset M
    M = M.cols(select1);
    p = M.n_cols;
    
    CharacterVector CpGS(p);
    for(int i = 0; i < p; ++i){
      CpGS[i] = "cg" + std::to_string(select1(i) + 1);
    }
    ...
    for (int i = 0; i < iter; ++i) {
      ...
      for (int m = 0; m < p; ++m) {
        ...
        double a = R::dnorm(betaM[m], 0, std::sqrt(SbetaM1),false) * PbetaM;
        double b = R::dnorm(betaM[m], 0, std::sqrt(SbetaM0),false) * (1 - PbetaM);
        PIPbetaM(m) = a / (a + b);
      ...
      PbetaM = R::rbeta(1 + sum(IbetaM), 1 + p - sum(IbetaM));
      
      //sampling SbetaM1 and SbetaM0
      SbetaM1 = 1/(R::rgamma(arma::accu(IbetaM) / 2 + 1, 1/(arma::accu(pow(IbetaM % betaM, 2)) / 2  + 1)));
      SbetaM0 = 1/(R::rgamma((p - arma::accu(IbetaM)) / 2 + 1, 1/(arma::accu(pow((1 - IbetaM) % betaM,  2)) / 2  + 10E-4)));
    ...
    //Assign CpG names as rownames
    DataFrame df_PIPb = DataFrame::create(Named("PIPbetaM") = PIP_b);
    df_PIPb.attr("row.names") = CpGS;
    
    //return necessary posteriors
    posterior[l] = List::create(Named("PIPbetaM") = df_PIPb, Named("betaM") = pos_betaM,
                                Named("PIPalphaT") = df_PIPa, Named("alphaT") = pos_alphaT);
