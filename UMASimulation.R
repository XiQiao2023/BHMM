                        ...
                        Beta0, Sbeta0, betaT, betaM, betaX,sigmaY)
{
  ...
  for (i in 1:N) {
    ...
    corrM = diag(p) + 0.05 * (matrix(1, p, p) - diag(p))
...
results = apply(M, 2, SingleMediation)

save(results,file = paste0("simulation9/M", p, "A", 0.1 * p, "N", n1 * n2,"C",c,"_", Sys.getenv("SLURM_ARRAY_TASK_ID"),".Rdata"))
