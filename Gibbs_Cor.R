...
Gibbs.Posterior = function(iteration, burn, chain){
  ...
  for (l in 1:chain) {
    ...
    for (i in 1:iter) {
      ...
      for (m in 1:p) {
        a = dnorm(alphaT[m], 0, sqrt(SalphaT1)) * PalphaT[m]
        b = dnorm(alphaT[m], 0, sqrt(SalphaT0)) * (1 - PalphaT[m])
        PIPalphaT[m] =  a / (a+b)
        IalphaT[m] = rbinom(1, 1, a / (a+b))
        PalphaT[m] = rbeta(1, 1 + IalphaT[m], 2 - IalphaT[m])
      }
    ...
    for (i in 1:iter) {
      ...
      for (m in 1:p) {
        ...
        a = dnorm(betaM[m], 0, sqrt(SbetaM1)) * PbetaM[m]
        b = dnorm(betaM[m], 0, sqrt(SbetaM0)) * (1 - PbetaM[m])
        PIPbetaM[m] = a / (a + b)
        IbetaM[m] = rbinom(1, 1, a / (a + b))
        PbetaM[m] = rbeta(1, 1 + IbetaM[m], 2 - IbetaM[m])
