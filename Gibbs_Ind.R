...
Gibbs.Posterior = function(iteration, burn, chain){
  ...
  for (l in 1:chain) {
    ...
    pos.Salpha0  = matrix(rep(1, 1 * iter), iter, 1, byrow = TRUE)
    
    pos.PalphaT  = matrix(rep(0, 1 * iter), iter, 1, byrow = TRUE)
    PIP.alphaT   = matrix(rep(1, p * iter), iter, p, byrow = T)
    pos.IalphaT  = matrix(rep(1, p * iter), iter, p, byrow = TRUE)
    pos.alphaT   = matrix(rep(0, p * iter), iter, p, byrow = TRUE)
    pos.SalphaT1 = matrix(rep(1, 1 * iter), iter, 1, byrow = TRUE)
    pos.SalphaT0 = matrix(rep(1, 1 * iter), iter, 1, byrow = TRUE)
    
    pos.sigmaM   = matrix(rep(1, 1 * iter), iter, 1, byrow = TRUE)
    
    ## Mediator Model Initial Value
    alpha0 = matrix(rep(0, p * n2), p, n2, byrow = TRUE)
    alphaX = matrix(rep(0, p * c) , p, c,  byrow = TRUE)
    for (m in 1:p) {
      alpha0[m,] = rep(0, n2)
      alphaX[m,] = rep(0, c)
    }
    Alpha0 = rep(0, p)
    Salpha0 = 1
    Mintercept = matrix(rep(0, p * n), n, p, byrow = TRUE)
